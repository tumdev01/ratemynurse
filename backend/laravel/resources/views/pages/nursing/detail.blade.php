...
<div class="p-4 sm:ml-64">
    ...
    <div class="p-4 mb-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 flex flex-row justify-between">
        <form id="detailNurse" class="text-[16px] flex flex-col gap-[32px] w-full max-w-[870px] mx-auto" method="POST" action="{{ route('nursing.detail.store', $nursing->id) }}" enctype="multipart/form-data">
            @csrf
            @if(session('error'))
            ...
            <div class="flex flex-col gap-[8px]">
                ...
                <textarea id="about" name="about" class="min-h-[90px] border rounded-lg px-3 py-2" placeholder="คำอธิบายรายละเอียดเกี่ยวกับบริการของคุณเพื่อให้ผู้ใช้บริการสนใจ">{{ old('extra_courses') }}</textarea>
                <span class="text-sm text-[#8C8A94]">คำอธิบายจะแสดงในหน้าข้อมูลบริการของคุณ กรุณาอธิบายรายละเอียดบริการของคุณ</span>
            </div>

            <div class="flex flex-col">
                <label class="mb-2 text-[#5A5A5A] text-medium" for="detail_images">รูปภาพเพิ่มเติม</label>
                ...
                <div class="border border-dashed rounded-lg h-[130px] flex justify-center items-center">
                    <div id="detail_upload" class="flex flex-row gap-[16px] justify-center items-center">
                        <img id="uploadAvatar" src="https://ratemynurse.org/wp-content/uploads/2025/08/upload2.png" loading="lazy" width="70" height="67">
                        <div class="flex flex-col gap-[8px]">
                            ...
                            <input type="file" id="hiddenDetailUpload" name="detail_images[]" multiple style="display:none">
                        </div>
                    </div>
                </div>
            ...
            <div class="gap-[16px] flex flex-col">
                ...
                <div class="flex flex-col flex-wrap gap-[16px]">
                    ...
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="ELEVATOR"
                            {{ in_array('ELEVATOR', $facilities_check) ? 'checked' : '' }}>
                        การดูแลผู้ป่วยติดเตียง
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="WHEELCHAIR_RAMP"
                            {{ in_array('WHEELCHAIR_RAMP', $facilities_check) ? 'checked' : '' }}>
                        การให้ยาและการฉีดยา
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="BATHROOM_GRAB_BAR"
                            {{ in_array('BATHROOM_GRAB_BAR', $facilities_check) ? 'checked' : '' }}>
                        การช่วยเหลือกิจกรรมประจำวัน (ADL)
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="EMERGENCY_BELL"
                            {{ in_array('EMERGENCY_BELL', $facilities_check) ? 'checked' : '' }}>
                        การดูแลแผลและแต่งแผล
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="CAMERA"
                            {{ in_array('CAMERA', $facilities_check) ? 'checked' : '' }}>
                        การใช้เครื่องทางการแพทย์
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="FIRE_SYSTEM"
                            {{ in_array('FIRE_SYSTEM', $facilities_check) ? 'checked' : '' }}>
                        การดูแลผู้ป่วยโรคเรื้อรัง
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="BACKUP_GENERATOR"
                            {{ in_array('BACKUP_GENERATOR', $facilities_check) ? 'checked' : '' }}>
                        การดูแลผู้ป่วยโรคอัลไซเมอร์/สมองเสื่อม
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="AIR_CONDITIONER"
                            {{ in_array('AIR_CONDITIONER', $facilities_check) ? 'checked' : '' }}>
                        การนวดและกายภาพบำบัด
                    </div>
                    <div class="flex flex-row gap-[8px] items-center text-[#1F1F1F]">
                        <input class="rounded-md border border-gray-200 w-5 h-5" type="checkbox" name="skills[]" value="GARDEN_AREA"
...
@section('javascript')
    ...
    <script>
        ...
        document.querySelector('#uploadAvatar').addEventListener('click', () => {
            document.getElementById('hiddenDetailUpload').click();
        });
